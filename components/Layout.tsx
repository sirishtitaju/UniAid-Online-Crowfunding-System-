...
import { LogOut, HeartHandshake, LogIn } from 'lucide-react';
...
export const Layout: React.FC<LayoutProps> = ({ user, onLogout, onLoginClick, children }) => {
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      <nav className="bg-emerald-700 text-white shadow-lg sticky top-0 z-50 transition-all">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            <div className="flex items-center cursor-pointer" onClick={() => !user && onLogout()}>
              <HeartHandshake className="h-8 w-8 text-emerald-300 mr-2" />
              <span className="text-2xl font-bold tracking-tight">UniAid</span>
            </div>
            ...
            {user ? (
              <div className="flex items-center space-x-4">
                <div className="hidden md:flex flex-col items-end mr-2">
                  ...
                  <span className="text-xs text-emerald-200 uppercase tracking-wider">{user.role}</span>
                </div>
        ...
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-8 text-sm">
           <div>
              <div className="flex items-center mb-4">
                 ...
                 <span className="text-lg font-bold text-slate-800">UniAid</span>
              </div>
              <p className="text-slate-500">
                  Empowering the next generation through secure, transparent, and community-driven crowdfunding.
              </p>
           </div>
           ...
           <div>
               ...
               <ul className="space-y-2 text-slate-500">
                   ...
                   <li><a href="#" className="hover:text-emerald-600">Trust & Safety</a></li>
           ...
           <div>
                <h4 className="font-bold text-slate-800 mb-4">Legal</h4>
                 <ul className="space-y-2 text-slate-500">
                   ...
                   <li><a href="#" className="hover:text-emerald-600">Fundraiser Guidelines</a></li>
               </ul>
           </div>
        </div>
        <div className="max-w-7xl mx-auto px-4 mt-8 pt-8 border-t border-slate-100 text-center text-slate-400 text-xs">
          &copy; {new Date().getFullYear()} UniAid Platform. All rights reserved.
        </div>
      </footer>
    </div>
  );
